        ...
        >
            ...
            (
                recents.map((recent)=>{
                    ...
                    recent.language != "Unknown" && accessories.unshift(
                        { tag: { value: recent.language, color: Color.Yellow }, icon: Icon.Code }
                    )
                    return (
                        <List.Item
                            key={recent.slug}
                            title={recent.slug}
                            subtitle={
                                recent.content.length > 25 ? 
                                    recent.content.replace("\n", "").substring(0, 50) + "..." 
                                    : 
                                    recent.content
                            }
                            icon={{source: "smc.svg"}}
                            ...
                            actions={
                                <ActionPanel title={`sharemycode.fr/${recent.slug}`}>
                                    ...
                                    <ActionPanel.Section title="Manage recents">
                                        <RemoveRecentAction onDelete={()=>{deleteRecent(recent.slug)}}/>
                                        <ClearRecentsAction onClear={()=>{clearRecents()}}/>
                                    </ActionPanel.Section>
                                </ActionPanel>
                            }
                        />
